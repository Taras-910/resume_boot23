...
import static ua.top.bootjava.aggregator.Installation.maxAge;
...
public class CommonUtil {
    ...
    public static String getLinkIfEmpty(String text) {
        return isEmpty(text) ? ConstantsUtil.link : text;
    }

    public static String getUpperStart(String text) {
        return !isEmpty(text) && text.length() > 1 ? getJoin(text.substring(0, 1).toUpperCase(),text.substring(1)) : ConstantsUtil.link;
    }
    ...
    public static String getLimitation(String text) {
        return isEmpty(text) ? ConstantsUtil.link : text.length() > maxLengthText ? text.substring(0, maxLengthText) : text;
    }

    public static boolean isAgeValid(String age) {
        return age.equals(ConstantsUtil.link) || Integer.parseInt(age.substring(0, age.indexOf(" "))) >= maxAge;
    }
